...
export const SearchBar: React.FC<{
  ...
  return (
    <div ref={searchRef} className="relative w-full max-w-3xl">
      <div
        className={`flex items-center bg-white rounded-xl border transition-all duration-200 ${
      ...
      >
        ...
        {query && (
          <button
            type="button"
            onClick={() => setQuery('')}
            className="p-1 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-100 mr-1"
          >
            <X className="w-4 h-4" />
          </button>
        )}
        ...
        <button
          ...
          className={`flex items-center justify-center rounded-xl text-white bg-[#ea580c] hover:bg-[#c2410c] transition-colors ${
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in-50 duration-150">
          {query.trim().length > 0 ? (
            <div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <img
                          src={prod.image}
                          alt={prod.name}
                          className="w-10 h-10 object-contain rounded-xl bg-white p-1 border border-slate-100"
                        />
                        <div>
              ...
              <div className="flex flex-wrap gap-2">
                {popularSearches.map((item) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-slate-100 hover:bg-orange-100 hover:text-[#c2410c] text-slate-700 rounded-xl transition-colors"
                  >
                    {item}
                  </button>
                ))}
